...
final class AppModel: ObservableObject {
    ...
    private let speechOutputCoordinator = SpeechOutputCoordinator()
    ...
    @Published private(set) var virtualMicrophoneState: SpeechOutputCoordinator.State = .disabled
    ...
    @Published var virtualMicrophoneEnabled: Bool {
        didSet {
            guard oldValue != virtualMicrophoneEnabled else { return }
            persistSettings()
    ...
    var virtualMicrophoneStatusText: String {
        let usesChinese = resolvedInterfaceLanguageID.hasPrefix("zh")
        ...
        case .speaking:
            return usesChinese ? "正在输出翻译语音" : "Sending translated speech"
        case .failed(let message):
            return usesChinese ? "输出失败：\(message)" : "Output failed: \(message)"
        }
    }

    var virtualMicrophoneSectionTitle: String {
        resolvedInterfaceLanguageID.hasPrefix("zh") ? "翻译虚拟麦克风" : "Translated Virtual Microphone"
    }

    var virtualMicrophoneToggleTitle: String {
        resolvedInterfaceLanguageID.hasPrefix("zh") ? "朗读翻译结果到虚拟麦克风" : "Speak translations into the virtual microphone"
    }

    var virtualMicrophoneTestTitle: String {
    ...
    private func speakCaptionIfNeeded(id: UUID, text: String, languageID: String) {
        guard virtualMicrophoneEnabled,
              text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false,
              spokenCaptionIDs.insert(id).inserted else {
            return
        }

        speechOutputCoordinator.enqueue(text: text, languageID: languageID)
        if spokenCaptionIDs.count > 200 {
            spokenCaptionIDs.removeAll(keepingCapacity: true)
    ...
    private func processCaptionQueue() async {
        ...
        while Task.isCancelled == false {
            ...
            speakCaptionIfNeeded(
                id: caption.id,
                text: resolvedTranslation,
                languageID: caption.targetLanguageID
            )
    ...
    private func applyLateCaptionTranslation(_ translatedText: String, for captionID: UUID) {
        ...
        if didApplyTranslation {
            ...
            let targetLanguageID = displayedCaption?.id == captionID
                ? displayedCaption?.targetLanguageID
                : pendingCaptions.first(where: { $0.id == captionID })?.targetLanguageID
            speakCaptionIfNeeded(
                id: captionID,
                text: translatedText,
                languageID: targetLanguageID ?? outputLanguageID
            )
        }
    ...
    private func sampleText(for languageID: String) -> String {
        switch languageID {
        case "zh-Hans":
            return "欢迎使用 TranslaMic，顶部字幕条已经准备好了。"
        case "es":
            return "Bienvenido a TranslaMic. La barra de subtitulos ya esta lista."
        case "de":
            return "Willkommen bei TranslaMic. Die Untertitel-Leiste ist bereit."
        case "ja":
            return "TranslaMic へようこそ。字幕バーの準備ができました。"
        case "fr":
            return "Bienvenue dans TranslaMic. La barre de sous-titres est prete."
        case "it":
            return "Benvenuto in TranslaMic. La barra dei sottotitoli e pronta."
        case "ko":
            return "TranslaMic에 오신 것을 환영합니다. 자막 바가 준비되었습니다."
        case "yue":
            return "歡迎使用 TranslaMic，字幕列已經準備好。"
        case "ar":
            return "مرحبا بك في TranslaMic. شريط الترجمة جاهز."
        case "pt":
            return "Bem-vindo ao TranslaMic. A barra de legendas esta pronta."
        case "ru":
            return "Добро пожаловать в TranslaMic. Строка субтитров готова."
        default:
            return "Welcome to TranslaMic. The subtitle bar is ready."
        }
    }
...
extension View {
    @ViewBuilder
    func translaMicTranslationHost(model: AppModel) -> some View {
