...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        .translaMicTranslationHost(model: model)
    ...
    private var virtualMicrophoneStatusColor: Color {
        switch model.virtualMicrophoneState {
        case .failed:
            return .red
    ...
    private var generalTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                settingsCard {
                    sectionHeader(model.virtualMicrophoneSectionTitle, icon: "mic.and.signal.meter")
                    settingsRow(model.virtualMicrophoneToggleTitle) {
                        Toggle("", isOn: $model.virtualMicrophoneEnabled)
                            .toggleStyle(.switch)
                            .labelsHidden()
                    }
                    Divider()
                    ...
                    HStack {
                        Button {
                            model.refreshVirtualMicrophoneState()
                        } label: {
                            Label(model.localized(.refreshSources), systemImage: "arrow.clockwise")
                        }
                        .disabled(model.virtualMicrophoneEnabled == false)

                        Spacer()

                        Button(model.virtualMicrophoneTestTitle) {
